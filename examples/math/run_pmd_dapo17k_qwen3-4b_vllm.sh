...
KL_LOSS_COEF=0.001  # not used since use_kl_loss=false
PPO_EPOCHS=1
SEQUENCE_PARALLEL=1
ROLLOUT_TP_SIZE=1
...
EXP_NAME=${EXP_NAME:-qwen3_4b_${POLICY_LOSS_MODE}ln_adv${ADV_ESTIMATOR}_stal${MAX_STALENESS}bsz512_mbsz32_n16_ep${PPO_EPOCHS}_t${PMD_TAU}_lr${ACTOR_LR}_ro${RESET_OPTIMIZER_FREQ}}

MODEL_PATH="Qwen/Qwen3-4B-Base"
DATA_DIR=${DATA_DIR:-"${HOME}/verl/data"}
TRAIN_DATA=${TRAIN_DATA:-"${DATA_DIR}/dapo-math-17k.parquet"}
VAL_DATA=${VAL_DATA:-"${DATA_DIR}/math-aime-eval.parquet"}
...
ray job submit --runtime-env="${RUNTIME_ENV}" \
    ...
    +actor_rollout_ref.actor.ppo_infer_max_token_len_per_gpu=${INFER_PPO_MAX_TOKEN_LEN_PER_GPU} \
